...
cdef class Species:
	"""Species(name, m_q, ppc, ufl=[0.,0.,0.], uth=[0.,0.,0.], density=None, n_sort=16, bc_type='periodic')
	...
	bc_type : {'periodic', 'open'}, optional
		Type of boundary conditions to be used for the species, must be
		one of 'periodic' (periodic boundaries), 'open' (absorbing
		boundaries), defaults to 'periodic'
	...
	_bc_types = {'periodic': PART_BC_PERIODIC,
				 'open':     PART_BC_OPEN }

	def __cinit__( self, str name, const float m_q, const int ppc, *,
				  list ufl = [0.,0.,0.], list uth = [0.,0.,0.],
				  Density density = None, int n_sort = 16,
				  str bc_type = 'periodic' ):
		...
		self._this.bc_type = self._bc_types[ bc_type ]
	...
	cdef new( self, t_species* ptr, int nx, float box, float dt ):
		self._thisptr = ptr
		# Store values overriden by spec_new()
		n_sort = self._this.n_sort
		bc_type = self._this.bc_type
	...
	def bc_type( self ):
		...
		"""
		tmp = { PART_BC_PERIODIC : 'periodic',
				PART_BC_OPEN : 'open' }
		return tmp[self._thisptr.bc_type]
...
cdef class EMF:
	...
	creating a `em1d.Simulation` object.
...
cdef class Current:
	...
	automatically when creating a `em1d.Simulation` object.
