...
class ZDFfile:
    ...
    def __read_xdr_string( self ):
        ...
        if ( length > 0 ):
            # Since strings are packed as xdr_bytes there is an additional length value
    ...
    def read_dataset( self ):
        ...
        if ( data_type == 9 ):
            ...
            data = np.array( self.__xdr.unpack_farray(size,self.__xdr.unpack_float),
                         dtype = np.float)
        elif ( data_type == 10 ):
            self.__xdr.reset( self.__file.read(size*8) )
            data = np.array( self.__xdr.unpack_farray(size,self.__xdr.unpack_double),
                         dtype = np.double)
        else:
...
def plot_dataset2D( dataset, grid_info, iteration ):
    ...
    colorMap = plotArea.imshow(dataset, cmap = plt.cm.jet, interpolation = 'nearest',
                           origin = 'lower')
    ...
    xlabel = grid_info['axis'][0]['label'] + '[' + grid_info['axis'][0]['units'] + ']'
    ylabel = grid_info['axis'][1]['label'] + '[' + grid_info['axis'][1]['units'] + ']'
